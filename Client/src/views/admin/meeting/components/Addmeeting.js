...
import { convertLocalToUTC } from "utils/utils";
...
const AddMeeting = (props) => {
  ...
  const AddData = async () => {
    // Converting the timsezone to utc here
    values.dateTime = convertLocalToUTC(values.dateTime);
    const data = await postApi("api/meeting/add", values);
    setAction(p => !p)
    onClose();
    return data;
  };
